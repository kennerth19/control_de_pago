...
<head>
    ...
    <link href="{{ asset('css/tuerquita.css') }}" rel="stylesheet">
...
<body id="body" style="background-image: url('/control_de_pago/public/img/background.jpg'); background-size: cover;">
    ...
    <div class="new_sec">
        ...
        <form>
            <select class="" name="by" id="by" onchange="this.form.submit()" onchange="status()">
                ...
                @endif
                @if($orde->status == "status")
                <option value="status" selected>Estado</option>
                @else
                <option value="status">Estado</option>
                @endif
                @if($orde->status == "destacated_com")
        ...
        <script>
            function select_type_insta() {
                ...
                for (let i = 1; i <= {{$var_regs}}; i++) { //OJO AQUI CUANDO TERMINES, TIENES QUE CAMBIAR ESE 1276 POR LA VARIABLE CANTIDAD DE CLIENTES
        ...
        <form class="can">
            <select name="cuenta" class="" id="cuenta" onchange="this.form.submit()" onchange="count()">
                ...
                @endif
                @if($orde->reg == $totales)
                <option value="{{$totales}}" selected>Todos</option>
                @else
                <option value="{{$totales}}">Todos</option>
                @endif
            </select>
        </form>
        <form class="">
    ...
    <ul class="ul" id="hover"  onmouseover="rotate(0)" onmouseleave="rotate(1)">
        <li class="li user"><a href="{{route('admin.panel')}}" class="loggine" alt="Administrar">Bienvenid@ {{ Auth::user()->email }}</a></li>
        <li class="li date_bar">Fecha del servidor: {{date("d-m-Y",strtotime(date('Y-m-d')))}}</li>
        <li class="li home"><a href="{{route('control')}}">Inicio</a><img class="ico_li" src='/control_de_pago/public/img/sidebar/ab.png'></li>
        <li class="li pay"><a href="{{ route('cliente.report') }}">Ver reporte general</a><img class="ico_li" src='/control_de_pago/public/img/sidebar/ab.png'></li>
        <li class="li cross"><a href="{{route('control.list')}}">Desactivados</a><img class="ico_li" src='/control_de_pago/public/img/sidebar/ab.png'></li>
        <li class="li mp"><a href="{{ route('cliente.pay.customer') }}">Ver reportes de pago</a><img class="ico_li" src='/control_de_pago/public/img/sidebar/ab.png'></li>
        <li class="li mp"><a href="{{ route('admin.bills') }}">Reporte de gastos</a><img class="ico_li" src='/control_de_pago/public/img/sidebar/ab.png'></li>
        ...
        <li>
            <div>
                <div class="sidebar_resume" style="display: none;">
                    <p class="font-bold">Desactivados: {{$u}}</p>
                    <p class="text-red-500 font-bold">Suspendidos: {{$s}}</p>
                    <p class="text-orange-500 font-bold">Prorroga 2 dias: {{$p_2}}</p>
                    <p class="text-orange-500 font-bold">Prorroga 1 dias: {{$p_1}}</p>
                    <p class="text-orange-500 font-bold">Resta 1 dia: {{$r_1}}</p>
                    <p class="text-orange-500 font-bold">Restan 2 dias: {{$r_2}}</p>
                    <p class="text-orange-500 font-bold">Dia de corte: {{$d}}</p>
                    <p class="text-green-500 font-bold">Solventes: {{$sol}}</p>
                    <p>Clientes sin numero: {{$cuenta_tlf}}</p>
                    <p>Activos totales: {{$count - $u}}</p>
                </div>
            </div>
        </li>
    </ul>
    ...
    <div class="header_index">
        <a href="{{route('control')}}" class="header_index_1" style="padding-left: 25px;"><span class="header_index_1_span_1">CONTROL DE </span><span class="header_index_1_span_2">PAGOS</span></a>
    ...
    <div class="form_new absolute left-1/2 transform -translate-x-1/2" id="form_new" style="display: none;">
        <form action="{{route('cliente.store')}}" method="POST" class="registro">
            @csrf
            <label>
                Nombre completo
                <input type="text" name="full_name" class="entrada" placeholder="Introduzca su nombre" value="{{old('full_name')}}">
            </label>
            <label>
                Cedula
                <br>
                <input type="text" id="id_user" name="id_user" class="entrada" placeholder="Introduzca su cedula" maxlength="8" value="{{old('id_user')}}" onkeypress="return valideKey(event);">
                <br>
                <small id="validacion" style="color: red; display:none;">Cedula registrada</small>
            </label>
            <label>
                Dirección
                <input type="text" name="dir" class="entrada" placeholder="Introduzca su Direccion" value="{{old('dir')}}">
            </label>
            <label>
                Telefono
                <br>
                <input type="text" id="tlf" name="tlf" class="entrada" placeholder="Introduzca un telefono" onkeypress="return valideKey(event);" value="{{old('tlf')}}">
            </label>
            <label>
                Fecha de instalación
                <input type="date" name="date_c" class="entrada" value="{{date('Y-m-d',strtotime(date('Y-m-d')))}}">
            </label>
            <label>
                IP Address <br>
                <input type="text" name="ip" class="entrada" placeholder="Introduzca la ip" value="{{old('ip')}}">
            </label>
            <label>
                Observación
            ...
            <label>
                ...
                @foreach($planes as $plan)
                    <option value="{{$plan->id}}">{{$plan->nombre_de_plan}}</option>
                @endforeach
            </select>
                </label>
            </div>
...
Default: 220$">Tipo de instalacion &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span><br>
            ...
            <label>
                Monto <br>
                <input type="number" name="monto_inst" class="entrada" placeholder="Monto de instalacion" value="{{old('monto_inst')}}">
            </label>
            <label>
    ...
    <div class="main_container border-teal-700" id="main_table" style="box-shadow: 2px 2px 5px #5b8dea; background-color: #fff;border-collapse: inherit; border-radius:15px;">
        ...
        <div class="child_container on" id="{{$contador}}">
            <div class="div_main main_item_1 container_id">
                <div class="main_item_1_server">
                    <p>{{$contador++}}</p>
                    <p style="display:none">{{$cliente->tlf}}</p>
                    <p style="display:none">123_{{$cliente->type}}</p>

                    @if($cliente->server_active == 1)
                        <form method="get" action="{{route('cliente.status_server')}}">
                            <input type="hidden" name="id" value="{{$cliente->id}}">
                            <input type="hidden" name="server_active" value="0">
                            @if(auth()->user()->role == 1 || auth()->user()->role == 2)
                                <input class="forma server_status_red" style="background-color: #ff0000;" type="submit" value="" title="{{date('d-m-Y',strtotime($cliente->last_cut_act))}}">
                            @else
                                <input class="forma server_status_red" style="background-color: #ff0000;" title="{{date('d-m-Y',strtotime($cliente->last_cut_act))}}">
                            @endif
                    </form>
                    @else
                        <form method="get" action="{{route('cliente.status_server')}}">
                            ...
                            @if(auth()->user()->role == 1 || auth()->user()->role == 2)
                                <input class="forma server_status_green" style="background-color: #27d719;" type="submit" value="" title="{{date('d-m-Y',strtotime($cliente->last_cut_act))}}">
                            @else
                                <input class="forma server_status_green" style="background-color: #27d719;" title="{{date('d-m-Y',strtotime($cliente->last_cut_act))}}">
                            @endif
            ...
            @if($cliente->destacated == 1)
                @if($cliente->total_debt > 0 && $cliente->data == 0) 
                    <p class="main_item_2 start_destacated"><img class="destacated" title="{{$cliente->destacated_com}}" src='/control_de_pago/public/img/start.png'><span style="color: #ff0000; cursor: pointer;" title="El cliente tiene una deuda de: {{$cliente->total_debt}}$ y su numero telefonico no esta registrado">{{ucwords(strtolower($cliente->full_name))}}</span></p>
                @elseif($cliente->total_debt > 0)
                    <p class="main_item_2 start_destacated"><img class="destacated" title="{{$cliente->destacated_com}}" src='/control_de_pago/public/img/start.png'><span style="color: #ff0000; cursor: pointer;" title="El cliente tiene una deuda de: {{$cliente->total_debt}}$">{{ucwords(strtolower($cliente->full_name))}}</span></p>
                @elseif($cliente->data == 0)
                    <p class="main_item_2 start_destacated"><img class="destacated" title="{{$cliente->destacated_com}}" src='/control_de_pago/public/img/start.png'><span style="color: #ff0000; cursor: pointer;" title="Numero telefonico no registrado">{{ucwords(strtolower($cliente->full_name))}}</span></p>
                @else
                    <p class="main_item_2 start_destacated"><img class="destacated" title="{{$cliente->destacated_com}}" src='/control_de_pago/public/img/start.png'><span>{{ucwords(strtolower($cliente->full_name))}}</span></p>
                @endif
            @else
                @if($cliente->total_debt > 0 && $cliente->data == 0)
                    <p class="main_item_2" style="color: #ff0000; cursor: pointer;" title="El cliente tiene una deuda de: {{$cliente->total_debt}}$ y su numero telefonico no esta registrado">{{ucwords(strtolower($cliente->full_name))}}</p>
                @elseif($cliente->total_debt > 0)
                    <p class="main_item_2" style="color: #ff0000; cursor: pointer;" title="El cliente tiene una deuda de: {{$cliente->total_debt}}$">{{ucwords(strtolower($cliente->full_name))}}</p>
                @elseif($cliente->data == 0)
                    <p class="main_item_2" style="color: #ff0000; cursor: pointer;" title="Numero telefonico no registrado">{{ucwords(strtolower($cliente->full_name))}}</p>
                @else
                    <p class="main_item_2">{{ucwords(strtolower($cliente->full_name))}}</p>
                @endif
            @endif
            <p class="main_item_3">{{ucwords(strtolower($cliente->dir))}}</p>
            <p class="main_item_4">{{ucwords(strtolower($cliente->id_user))}}</p> 
            <a href="https://{{$cliente->ip}}" class="main_item_5" target="_blank">{{$cliente->ip}}</a>
            <p class="main_item_6">{{date("d-m-Y",strtotime($cliente->cut))}}</p>
            <p class="main_plan main_item_7">{{$cliente->plan}}</p>
            @if($cliente->status == 1)
            ...
            @elseif($cliente->status == 7 and $cliente->active == 1)
            ...
            @endif
                <a href="{{route('cliente.pay',$cliente->id)}}" class="acciones main_item_10" target="_blank">Pagar</a>
                <button onclick="iframe_src({{$cliente->id}})" class="acciones main_item_11">Modificar</button>
        </div>
        @endforeach
    </div>
