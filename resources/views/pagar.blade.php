...
@if(auth()->user()->role == 1 || auth()->user()->role == 2 && auth()->user()->email == 'antonio' || auth()->user()->email == 'jean' || auth()->user()->email == 'taquilla' || auth()->user()->email == 'marco')
<style>
    .form{
        display: grid;
    ...
    <div class="grid-container">
        <div class="grid-item">
            ...
            <script>
                ...
                <form action="{{route('destacated',$users->id)}}" class="destacated_form" style="display: none;">
                    <textarea name="destacated" id="" cols="30" rows="10"></textarea><br>
                    <input type="submit" value="Destacar">
                </form>
            @else
                <a href="{{route('destacated',$users->id)}}" class="">Quitar destacado</a>
            @endif
            
            <p>Nombre: {{$users->full_name}} <a href="{{route('cliente.edit',$users->id)}}" class="modify_pay" target="_blank">Modificar</a></p>
            <p>Cedula: {{$users->id_user}}</p>
            <p>Telefono: {{$users->tlf}}</p>
            <p>Direccion: {{$users->dir}}</p>
            <p>Fecha de suspension actual: {{date('d-m-Y',strtotime($users->cut))}}</p>
            <form action="{{route('cliente.pay_update',$users)}}" class="formulario-pagar" method="POST" class="w-full max-w-lg m-5">
                @method('put')
                @csrf
                <input type="hidden" name="status" value="1">
                <input type="hidden" name="id_user" value="{{$users->id_user}}">
                <input type="hidden" name="plan_ab" value="{{$users->total}}">
                <p>Codigo de cliente: {{$users->id}}</p>
                <input type="hidden" name="dir" value="{{$users->dir}}">
                <label>
                    Cobrador:
                    <select name="reg">
                        <option value="marco escala">Marco Escala</option>

                        @if(auth()->user()->email == 'antonio')
                ...
                <label>
                    ...
                    @else
                    <input type="date" name="day" value="{{date('Y-m-d',strtotime(date('Y-m-d')))}}">
                    @endif
                    @error('day')
  ...
  function type_pay_a() {
    ...
    if (x == "zb") {// zelle b
      ...
      type_pay_zb.style.display = "none";
                ...
                <input type="hidden" name="cut" value="{{date('Y-m-d', strtotime($users->cut))}}">

                <input type="hidden" name="plan" disabled value="{{$users->plan}}">
...
<label>
    <input type="text" id="in_d" min="1" max="{{$users->total}}" name="d" value="" placeholder="Dolares" onkeypress="return valideKey(event);" style="display:none;"/>
    <input type="text" id="bs_pm" min="1" max="{{$users->total}}" name="bs_pm" value="" placeholder="Bolivares (efectivo)" onkeyup="tasa()" onkeypress="return valideKey(event);" style="display: none;"/>
    <input type="text" id="bs" min="1" max="{{$users->total}}" name="bs" value="" placeholder="Pagomovil" onkeyup="tasa()" onkeypress="return valideKey(event);" style="display: none";/>
    <input type="text" id="in_za" min="1" max="{{$users->total}}" name="za" value="" placeholder="Zelle A" onkeypress="return valideKey(event);" style="display: none;"/>
    <input type="text" id="in_zb" min="1" max="{{$users->total}}" name="zb" value="" placeholder="Zelle B" onkeypress="return valideKey(event);" style="display: none;"/>
    ...
    <small id="show_alert" style="color: red; display: none;">Referencia del pagomovil obligatoria y debe estar en entre el rango 10000 - 99999*</small>
                ...
                <style>
                    ...
                    <div class="calculadora">
                        ...
                        <h1>Tasa actual: {{$tasa->tasa}}Bs.</h1>
                        <h1 id="h1"></h1>
                        <h1 id="h2"></h1>
                        <h1 id="h3"></h1>
                    </div>
                    ...
                    <script>
                        function tasa() {
                            ...
                            let cambio = parseFloat((bs.value / {{$tasa->tasa}}).toFixed(2));
                            let iva = parseFloat(((bs.value * 0.16) / {{$tasa->tasa}}).toFixed(2));
                            let total_bs = parseFloat(cambio + iva);
                            let total_d = parseFloat(bs.value * 0.16) + parseFloat(bs.value);

                            if(bs.value > 0){
                                h1.innerHTML = "Al cambio BCV: " + (bs.value / {{$tasa->tasa}}).toFixed(2) + "$";
                                h2.innerHTML = "16% de Iva: " + (bs.value * 0.16).toFixed(2) + "Bs." + " (" + ((bs.value * 0.16) / {{$tasa->tasa}}).toFixed(2) + " $)";
                ...
                <label>
                    ...
                    <textarea class="border border-slate-300" name="observation" style="resize:none;" id="" cols="25" rows="5">{{$users->observation}}</textarea>
                    @error('observation')
                ...
                <label>
                    ...
                    <textarea class="border border-slate-300" name="comentario_instalador" style="resize:none;" id="" cols="25" rows="5">{{$users->comentario_instalador}}</textarea>
                </label>
                ...
                <br>
                <label>
                    Fecha de suspensión (Despues del pago): {{date('d-m-Y',strtotime($users->cut .'+ 1 month'))}}
                </label>
                <br><br>
                <label>
                    <p>El cliente tiene una deuda total por soportes de : {{$users->total_deb_support}} <br> Nota: este valor del soporte se vera reflejado en el ticket</p>
                </label>
                <p>El monto del plan de {{$users->plan}} es: {{$users->total}}$</p>
                @if($users->total_debt_m <= 0) <p>El cliente ha abonado un total de: 0$</p>
                    <p>El total a pagar es: <span style="color: red;">{{$users->total - $users->total_debt_m}}$</span></p>
                    @else
                    <p>El cliente ha abonado un total de: {{round($users->total - $users->total_debt_m ,2)}}$</p>
                    <p>El total a pagar es: <span style="color: red;">{{round($users->total - ($users->total - $users->total_debt_m),2)}}$</span></p>
                    @endif

                    <input type="hidden" name="total_ab" value="{{$users->total - $users->total_debt_m}}">
        ...
        <div class="grid-item">
            <table class="tabla text-center z-10 rounded border-teal-700" style="box-shadow: 2px 2px 5px #5b8dea;">
                ...
                <tr>
                    ...
                    <td>Diciembre</td>
                    @endif
                    @if($pago->paid == "Pagado" and $pago->repa === "1")
                    <td class="border border-slate-300" style="color:green">{{$pago->paid}}</td>
                    <td>
                        <form id="form_ac" method="post" action="{{route('admin.pay_act_des',$pago->id)}}">
                            @csrf
                            <input type="submit" class="act_des" name="de" value="Desactivar" onclick="this.form.submit()">
                        </form>
                    </td>
                    @elseif($pago->paid == "Sin pagar" and $pago->repa === "1")
                    <td style="color:red">{{$pago->paid}}</td>
                    <td>
                        <form id="form_ac" method="post" action="{{route('admin.pay_act_des',$pago->id)}}">
                            @csrf
            ...
            <a href="{{route('cliente.add_month',$users->id)}}" style="display:none;" id="meses">Agregar meses</a>
            <script>
                function nuke(){
        ...
        <div class="deuda_pagos">
            <div>
            @if($users->total_debt == 0 or $users->total_debt < 0) <h1>Deuda del cliente</h1>
                <p>El cliente esta solvente con el pago de su instalacion</p>
                @else
                <p>El cliente tiene una deuda de <span style="color: red;">{{round($users->total_debt,2)}}$</span></p>
                <form action="{{route('cliente.pay_debt',$users->id)}}" class="service_pay" method="post">
                    @csrf
                    @method('put')
                    <input type="hidden" name="name" value="{{$users->full_name}}">
                    <input type="hidden" name="dir" value="{{$users->dir}}">
                    <label>
                        Concepto<br>
                        <input type="text" name="conc" value="Pago de {{round($users->total_debt,2)}}$ de la instalacion" required>
                    </label><br>
                    <label>
                    Cobrador:
                    <select name="reg">
                    ...
                    <div class="calculadora" id="calculadora">
                        ...
                        <script>
                            function ref_deb() {
                                ...
                                let cambio = parseFloat((bs_deb.value / {{$tasa->tasa}}).toFixed(2));
                                let iva = parseFloat(((bs_deb.value * 0.16) / {{$tasa->tasa}}).toFixed(2));
                                let total_bs = parseFloat(cambio + iva);
                                let total_d = parseFloat(bs_deb.value * 0.16) + parseFloat(bs_deb.value);

                                if(bs_deb.value > 0){
                                    h1_dep.innerHTML = "Al cambio BCV: " + (bs_deb.value / {{$tasa->tasa}}).toFixed(2) + "$";
                                    h2_dep.innerHTML = "16% de Iva: " + (bs_deb.value * 0.16).toFixed(2) + "Bs." + " (" + ((bs_deb.value * 0.16) / {{$tasa->tasa}}).toFixed(2) + "$)";
                ...
                <div class="pagos_hechos">
                    ...
                    @foreach($pagos_realizados as $realizado)
                        <tr>
                            <td>{{$realizado->monto_d+0}}</td>
                            <td>{{$realizado->monto_bs+0}}</td>
                            <td>{{$realizado->monto_z_1+0}}</td>
                            <td>{{$realizado->monto_z_2+0}}</td>
                            <td>{{$realizado->fecha_pago}}</td>
                        </tr>
                    @endforeach
                    </table>  
                </div>
        ...
        <div class="soportes">
                ...
                @if($users->id_user == $support_item->id_user)
                    <tr>
                        <td>{{$i++}}</td>
                        <td>{{$support_item->id_user}}</td>
                        <td>{{$support_item->nota}}</td>
                        <td>{{$support_item->total+0}}$</td>
                        <td>{{$support_item->fecha}}</td>
                        @if($support_item->active == 0 )
                            <td>
                                <form action="{{route('ac_sup',$support_item->id)}}" method="post">
                                    @csrf
                                    <input type="submit" value="Activar">
                                </form>
                            </td>
                        @elseif($support_item->active == 1)
                            <td>
                                <form action="{{route('des_sup',$support_item->id)}}" method="post">
                                    @csrf
                        ...
                        @endif
                        <td>
                            <form action="{{route('del_sup',$support_item->id)}}" method="post">
                                @csrf
    ...
    <form action="{{route('cliente.update_meses',$users->id)}}" method="post" class="form" id="meses_form" style="display:none">
        @method('put')
        @csrf
        <input type="hidden" value="{{$users->id}}" name="usuario">
        <label for="">
            abono
    ...
    <script>
        Swal.fire({
            ...
            color: '#000',
            footer: '<a href="{{route("cliente.fac",$factura)}}" class="print_fac">Imprimir factura</a>',
        })
    </script>
    @endif
...
@if(session('form') == "ok")
        <script>
            ...
            html: `<form action="{{route('cliente.update_meses',$users->id)}}" method="post" id="meses_form">
                  @method('put')
                  @csrf
                  <input type="hidden" value="{{$users->id}}" name="usuario">
